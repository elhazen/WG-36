...
dataset <-1996 #use either the "1996" or "2003" datasets for the California Current indicators. 1996 dataset includes data from 1996-2014; truncated dataset contains data from 2003-2102 - this is used in comparison with gradient forest dataset
...
GAM.bootstrap <- "gaussian" #use either "residual" or "gaussian". These are two methods of calculating the confidence intervals for relationships in which GAM is the best model. This will trigger ifelse statements to use "resids" or "resids.use" when calculating the CI for the main GAM smoother, 1st derivative and 2nd derivative that are produced in the 3-panel figures. "gaussian" is used to try to use the same method as used to calculate the CIs for GAMM models. Initially, I thought using this method would produce more visible differences where the CIs did not include 0, but alas (and probably re-assuring) the CIs are nearly identical for each method.
...
computer <- "laptop"  #if using "laptop" or "desktop"

desktop <- "/Users/kelly.andrews/Documents/IEAs/California Current/Human Activities data/SoCC report/2018 report/reference points"
laptop <- "/Users/kelly.andrews/Documents/2018 report/Reference points"
...
data.dir <- "data/"
if (file.exists(data.dir)==FALSE) dir.create(data.dir)
results.dir <- "output/"
...
ifelse(dataset==1996,{
        cc <- read.csv("data/1996.2016.reference.points.data.withNOIsummerJuneAugust.csv")
        ccALL <- cc
        ...
        ##############################################################
        ifelse(test=="no",{
          resp <- c(rep("CA sea lion pup production",2), 
                    rep("Copepod anomaly summer",2)
                    )
          dri <- c(rep(c("NOI summer","NPGO winter"),2))
                          },  #end "no" statement
          {
          #TO TEST CODE WITH ONE COMBINATION. ONLY USED IF "test" is set to "yes" in Instructions at top of code
          #resp <- c(rep("Groundfish diversity",16))  #set the number value to the number of drivers being used in this test
          ...
          #                "NOI summer","NOI winter","NPGO summer","NPGO winter","PDO summer","PDO winter"),1))  #set the number to the number of responses being used in this test
        ...
        },  #end of 1996 variables
       {
        ...
        ifelse(test=="no",{
          resp <- c(rep("CA sea lion pup growth",14),rep("CA sea lion pup production",14),
                  ...
                  rep("Groundfish species richness",14), rep("Groundfish diversity",14),
...
# # Pull out relevant model outputs:
    ...
    summary.gamm1$best.model = ifelse(as.numeric(as.character(summary.gamm1$p.ac))<=0.05,
                                    ifelse(as.numeric(as.character(summary.gamm1$edf))>=1.99,
                                           ifelse(as.numeric(as.character(summary.gamm1$delta.AIC))>=2.0,"yes","no"),"no"),"no")
...
allSummary <- rbind(allSummary, summary.gamm1, summary.lmac, summary.gam1,
                    summary.linear)
...
if(test=="no")
  ...
  write.csv(allSummary,paste0(results.dir,paste0(dataset,".all.models.summary.out.csv")),row.names=F)
...
####To get non-significant non-linear relationships for sea lion pup production (e.g., to compare to significant results for pup production), use the following code for "pick.gamm" and "pick.gam"
#pick.gamm <- which(allSummary$RESPONSE=="CA sea lion pup production" & allSummary$best.model=="no" & allSummary$MODEL=="GAMM")
#pick.gam <- which(allSummary$RESPONSE=="CA sea lion pup production" & allSummary$best.model=="no" & allSummary$MODEL=="GAM")
...
for(row.i in pick.gamm) {    
  ...
  pred <- predict.gam(gamm1$gam, newdata = ind.fit, type = "response", se.fit = T)
  ...
  resids <- myresponse-gamm1$gam$fitted
  
  # sample multivariate normal for use in gaussian process regression bootstrap
  phi = coef(gamm1$lme$modelStruct$corStruct,unconstrained=FALSE)
  ...
  for (i in 1:nb)
  {
    ...
    #respboot[,i] <- gamm1$gam$fitted + sample(resids,length(resids),replace=TRUE)
    
    #use next line if doing gaussian process residual bootstrap - for "gamm" calls
    respboot[,i] <- gamm1$gam$fitted + resids.use[i,]
    drivboot[,i] <- mydriver
  }
...
for(row.i in pick.gam) {
  ...
  ###This is an attempt to reproduce the gaussian process regression bootstrap for the GAM...Figures aren't much better...not sure if this is better than just normal residual bootstrapping
  dist <- diag(length(resids))
  for (i in 1:nrow(dist)) for (j in 1:ncol(dist))
  resids.gaus <- rmvnorm(nb,rep(mean(resids),length(resids)),sigma=var(resids)*dist,method="chol")
  

 #Use the following code for CI bootstrapping 
   for (i in 1:nb)
   {
     ifelse(GAM.bootstrap=="residual", 
            (respboot[,i] <- gam1$fitted + sample(resids,length(resids),replace=TRUE)),
            (respboot[,i] <- gam1$fitted + resids.gaus[i,]))
     drivboot[,i] <- mydriver
   }
